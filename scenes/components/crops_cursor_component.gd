...
var player : Player
var corn_plan_scene = preload("res://scenes/objects/plants/corn.tscn")
var tomato_plan_scene = preload("res://scenes/objects/plants/tomato.tscn")
...
func get_cell_under_mouse() -> void:
	...
	# Get the source id of the tile, returns -1 if clicked outside the tilled_soil_tilemap_layer
...
func add_crop() -> void:
	# check if there is already a crop on this field if so, return
	
	if distance < 20.0:
		if ToolManager.selected_tool == DataTypes.Tools.PlantCorn:
			var corn_instance = corn_plan_scene.instantiate() as Node2D
		...
		if ToolManager.selected_tool == DataTypes.Tools.PlantTomato:
			var tomato_instance = tomato_plan_scene.instantiate() as Node2D
