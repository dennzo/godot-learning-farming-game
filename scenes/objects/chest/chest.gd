...
@onready var feed_component: Area2D = $FeedComponent
...
func _unhandled_input(event: InputEvent) -> void:
	if in_range:
		if event.is_action_pressed('show_dialogue'):
			...
			balloon.start(load('res://dialogue/coversations/chest.dialogue'), dialogue_start_command)
...
func trigger_feed_harvest(inventory_item: String, scene: Resource) -> void:
	...
	for index in inventory_item_count:
		...
		# todo: disable collision shape so player cannot interact with it.
		#print(harvest_instance.collision_layer)
...
# calculate random position within the marker "center" for reward
